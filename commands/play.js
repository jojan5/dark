...
module.exports = {
	...
	execute: async ({ client, interaction }) => {
		...
		if (interaction.options.getSubcommand() === "song") {
            ...
            if (result.tracks.length === 0)
                return interaction.reply(`no ecnontre la playlist${url}`)
            
            // Add the tracks to the queue
            const playlist = result.playlist
            await queue.addTracks(result.tracks)
            embed
                .setDescription(`**${result.tracks.length} cancion de [${playlist.title}](${playlist.url})** ha sido agregada a la lista`)
...
*/
interaction.reply('buscando musica de youtube onii-san')
